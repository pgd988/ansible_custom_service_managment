...
1. **Generate a deployment key-pair:**
   ...
   ```
2. **Encrypt the private key with Ansible Vault:**
   ```bash
   ansible-vault encrypt ci_ssh_key
   ```
   *(You will be prompted to create a vault password. Remember this password.)*
3. **Commit the encrypted key:**
   Add the encrypted `ci_ssh_key` file to your repository. (Do **NOT** commit the unencrypted version!)
4. **Provision the public key:**
   Rename the generated `ci_ssh_key.pub` to something like `ci_user.pub`, place it into `roles/default/files/pub_keys/ci_user.pub`, and add the `ci_user` account to your `users_list.yaml` so the server will accept connections from the pipeline.
